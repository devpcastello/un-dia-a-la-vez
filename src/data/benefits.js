import dulcinea1 from "../assets/dulcinea-1.png";
import dulcinea2 from "../assets/dulcinea-2.png";

export const benefits = [
  {
    title: "No hay límites geográficos",
    image: `${dulcinea1}`,
    description: `
  Además de ser cómoda y flexible, la terapia online también puede ser una excelente opción para aquellos que tienen dificultades para asistir a terapias en persona debido a sus compromisos diarios, como el trabajo o los estudios. La terapia en línea también puede ser más accesible para personas que viven en áreas remotas o que tienen dificultades para movilizarse debido a una discapacidad o problemas de salud.
  `,
  },
  {
    title: "Flexibilidad en las consultas",
    image: `${dulcinea2}`,
    description: `
    La terapia online ofrece una gran flexibilidad en las consultas, ya que puedes elegir el horario que mejor se adapte a tus necesidades y compromisos. Además, puedes tener acceso a tu terapeuta desde cualquier lugar del mundo, lo que significa que no tienes que preocuparte por las barreras geográficas. Esta flexibilidad también te permite ajustar la frecuencia de las sesiones en función de tu disponibilidad, lo que te ayuda a mantener el equilibrio entre tus responsabilidades y tu bienestar emocional.
    `,
  },
  {
    title: "Un entorno seguro y de confianza",
    image: `${dulcinea2}`,
    description: `
    En un entorno seguro y de confianza, podrás compartir tus experiencias y sentimientos sin miedo a ser juzgado. La privacidad de la terapia online garantiza la confidencialidad y el respeto hacia tus necesidades personales. Además, contar con la comodidad de un entorno conocido puede ayudarte a sentirte más relajado y abierto durante la terapia.
    `,
  },
  {
    title: "El mismo resultado que una terpia presencial",
    image: `${dulcinea2}`,
    description: `
    La terapia online ofrece el mismo resultado que una terapia presencial, con la ventaja de poder realizarla desde la comodidad de tu hogar. Al ser una terapia basada en la comunicación, la efectividad de la misma no se ve comprometida por la distancia física. Además, contar con la tecnología actual permite a los terapeutas utilizar herramientas digitales para mejorar el proceso terapéutico y hacerlo más ameno.
    `,
  },
];
